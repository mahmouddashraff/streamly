...
export default function PresenterContentPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  async function fetchData() {
    // Fetch Presenter
    const { data: eData } = await supabase.from('presenters').select('*').eq('id', id).single();
    if (eData) setPresenter(eData);

    // Fetch associated videos
    const { data: vData } = await supabase.from('videos').select('*').eq('presenter_id', id).order('created_at', { ascending: false });
  ...
  if (!Presenter) return <div className="p-8 text-red-500">Presenter not found</div>;
  ...
  return (
    <div className="p-8">
      ...
      <div className="flex items-center gap-6 mb-12 bg-card p-6 rounded-xl border border-border">
        <img src={Presenter.thumbnail} alt={Presenter.title} className="w-32 h-32 object-cover rounded-lg shadow-md" />
        <div>
          <h1 className="text-3xl font-bold tracking-tight mb-2">{Presenter.title}</h1>
          <p className="text-muted-foreground max-w-2xl">{Presenter.description}</p>
        </div>
      </div>
      ...
      <div className="flex items-center justify-between mb-8">
        ...
        <Link 
          href={`/admin/presenters/${id}/content/new`}
              ...
              <tr key={video.id} className="hover:bg-muted/50 transition-colors">
                ...
                <td className="p-4 text-right">
                  <div className="flex items-center justify-end gap-2">
                    <Link 
                      href={`/admin/presenters/${id}/content/${video.id}/edit`}
            ...
            {videos.length === 0 && (
              <tr>
                <td colSpan={3} className="p-8 text-center text-muted-foreground">
                  No videos found for this Presenter. Click "Add Video" to create one.
                </td>
              </tr>
            )}
          </tbody>
