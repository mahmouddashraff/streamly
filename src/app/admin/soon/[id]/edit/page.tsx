...
export default function EditSoonPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    async function fetchData() {
      const { data: soonData, error } = await supabase.from('soon').select('*').eq('id', id).single();
      if (soonData && !error) {
        setOriginalThumbnailUrl(soonData.thumbnail);
        setFormData({
          title_en: soonData.title_en || soonData.title,
          title_ar: soonData.title_ar || soonData.title,
          description_en: soonData.description_en || soonData.description,
          description_ar: soonData.description_ar || soonData.description,
          thumbnail: soonData.thumbnail,
          published: soonData.published,
        });
      } else {
        alert("Failed to load soon data.");
      }
      setIsLoading(false);
    }
    fetchData();
  ...
  if (isLoading) {
    return <div className="p-8 text-center text-muted-foreground">Loading soon data...</div>;
  }

  return (
    <div className="p-8 max-w-4xl mx-auto">
      <h1 className="text-3xl font-bold tracking-tight mb-8">Edit Soon Show</h1>
      
      <div className="bg-card border border-border rounded-xl p-6">
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-2">
              <label htmlFor="title_en" className="text-sm font-medium">Soon Title (English)</label>
              <input
                id="title_en"
                name="title_en"
                required
            ...
            <div className="space-y-2">
              <label htmlFor="title_ar" className="text-sm font-medium">Soon Title (Arabic)</label>
              <input
                id="title_ar"
                name="title_ar"
                required
