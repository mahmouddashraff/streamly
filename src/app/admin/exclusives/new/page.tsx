...
export default function AddExclusivePage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { error: dbError } = await supabase.from('exclusives').insert(newExclusive);
  ...
  return (
    <div className="p-8 max-w-4xl mx-auto">
      <h1 className="text-3xl font-bold tracking-tight mb-8">Create New Exclusive Show</h1>
      ...
      <div className="bg-card border border-border rounded-xl p-6">
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="pt-6 border-t border-border flex justify-end gap-4">
            ...
            >
              {isUploading ? "Saving..." : "Create Exclusive"}
            </button>
          </div>
        </form>
      </div>
