...
export default async function TodaysEventPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = await params;
  const supabase = await createClient();

  const { data: todaysEvent, error } = await supabase
    .from('todays_events')
    .select('*')
    .eq('id', id)
    .single();

  if (error || !todaysEvent) {
    return notFound();
  }
  ...
  const title = getLocalizedField(todaysEvent, 'title', locale);
  const description = getLocalizedField(todaysEvent, 'description', locale);

  return (
    <main className="min-h-screen bg-background pt-24 pb-12">
      <div className="container mx-auto px-4 md:px-8">
        
        {/* Today's Event Header */}
        <div className="flex flex-col md:flex-row items-center md:items-start gap-8 mb-16 bg-white/5 p-8 rounded-2xl border border-white/10 backdrop-blur-sm">
          <div className="relative w-full md:w-64 aspect-video md:aspect-square rounded-xl overflow-hidden flex-shrink-0 shadow-2xl border border-white/10">
            {todaysEvent.thumbnail ? (
              <img 
                src={todaysEvent.thumbnail} 
                alt={title} 
