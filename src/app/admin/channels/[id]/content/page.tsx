...
export default function ChannelContentPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  async function fetchData() {
    // Fetch Channel
    const { data: eData } = await supabase.from('channels').select('*').eq('id', id).single();
    if (eData) setChannel(eData);

    // Fetch associated videos
    const { data: vData } = await supabase.from('videos').select('*').eq('channel_id', id).order('created_at', { ascending: false });
  ...
  if (!Channel) return <div className="p-8 text-red-500">Channel not found</div>;
  ...
  return (
    <div className="p-8">
      ...
      <div className="flex items-center gap-6 mb-12 bg-card p-6 rounded-xl border border-border">
        <img src={Channel.thumbnail} alt={Channel.title} className="w-32 h-32 object-cover rounded-lg shadow-md" />
        <div>
          <h1 className="text-3xl font-bold tracking-tight mb-2">{Channel.title}</h1>
      ...
      <div className="flex items-center justify-between mb-8">
        ...
        <Link 
          href={`/admin/channels/${id}/content/new`}
              ...
              <tr key={video.id} className="hover:bg-muted/50 transition-colors">
                ...
                <td className="p-4 text-right">
                  <div className="flex items-center justify-end gap-2">
                    <Link 
                      href={`/admin/channels/${id}/content/${video.id}/edit`}
            ...
            {videos.length === 0 && (
              <tr>
                <td colSpan={3} className="p-8 text-center text-muted-foreground">
                  No videos found for this Channel. Click "Add Video" to create one.
                </td>
              </tr>
            )}
          </tbody>
