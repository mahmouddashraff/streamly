...
export default function EditTodaysEventPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    async function fetchData() {
      const { data: todaysEventData, error } = await supabase.from('todays_events').select('*').eq('id', id).single();
      if (todaysEventData && !error) {
        setOriginalThumbnailUrl(todaysEventData.thumbnail);
        setFormData({
          title_en: todaysEventData.title_en || todaysEventData.title,
          title_ar: todaysEventData.title_ar || todaysEventData.title,
          description_en: todaysEventData.description_en || todaysEventData.description,
          description_ar: todaysEventData.description_ar || todaysEventData.description,
          thumbnail: todaysEventData.thumbnail,
          published: todaysEventData.published,
          });
      } else {
        alert("Failed to load todaysEvent data.");
      }
      setIsLoading(false);
    }
    fetchData();
  ...
  if (isLoading) {
    return <div className="p-8 text-center text-muted-foreground">Loading todaysEvent data...</div>;
  }

  return (
    <div className="p-8 max-w-4xl mx-auto">
      <h1 className="text-3xl font-bold tracking-tight mb-8">Edit Today's Event Show</h1>
      
      <div className="bg-card border border-border rounded-xl p-6">
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-2">
              <label htmlFor="title_en" className="text-sm font-medium">Today's Event Title (English)</label>
              <input
                id="title_en"
                name="title_en"
                required
            ...
            <div className="space-y-2">
              <label htmlFor="title_ar" className="text-sm font-medium">Today's Event Title (Arabic)</label>
              <input
                id="title_ar"
                name="title_ar"
                required
            ...
            <div className="space-y-4 md:col-span-2 p-4 border border-border rounded-xl bg-muted/30">
              <label className="text-sm font-medium block mb-2">Today's Event Cover/Thumbnail</label>
