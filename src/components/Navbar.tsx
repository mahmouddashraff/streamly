...
export default function Navbar({ settings }: { settings?: any }) {
  ...
  return (
    ...
    >
      <div className="container mx-auto px-3 sm:px-4 md:px-8 flex items-center justify-between gap-2">
        {/* Left Side: Mobile Hamburger + Logo + Desktop Nav */}
        <div className="flex items-center gap-2 sm:gap-3 lg:gap-8 xl:gap-12 flex-1 min-w-0">
          ...
          <Link href="/" className="flex items-center gap-2 sm:gap-4 group min-w-0 flex-shrink-0">
            {activeLogo && (
              <img 
                src={activeLogo} 
                alt={activeSiteName} 
                className="h-6 xs:h-7 sm:h-12 md:h-16 lg:h-12 xl:h-14 w-auto object-contain transition-transform group-hover:scale-105 flex-shrink-0"
              />
            )}
            <div className="flex flex-col items-start text-start min-w-0">
              <span 
                className="text-sm xs:text-base sm:text-2xl md:text-3xl lg:text-2xl xl:text-3xl font-extrabold transition-transform group-hover:scale-105 group-active:scale-95 whitespace-normal lg:whitespace-nowrap leading-tight break-words max-w-[120px] sm:max-w-none" 
                style={{ fontFamily: '"Alpha Eco", system-ui, sans-serif', color: '#800020' }} 
              >
                {activeSiteName}
              </span>
              <span 
                className="hidden sm:block text-xs sm:text-lg md:text-xl lg:text-lg xl:text-xl font-medium tracking-wide mt-0 sm:mt-2 transition-transform group-hover:scale-105 truncate w-full"
          ...
          {/* Desktop Nav */}
          <nav className="hidden lg:flex items-center gap-4 xl:gap-6 flex-nowrap flex-shrink-0">
            {navLinks.map((link) => (
              <Link
                key={link.name}
                href={link.path}
