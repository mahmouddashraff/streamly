...
export default function EditPodcastPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    async function fetchData() {
      const { data: podcastData, error } = await supabase.from('podcasts').select('*').eq('id', id).single();
      if (podcastData && !error) {
        setOriginalThumbnailUrl(podcastData.thumbnail);
        setFormData({
          title_en: podcastData.title_en || podcastData.title,
          title_ar: podcastData.title_ar || podcastData.title,
          description_en: podcastData.description_en || podcastData.description,
          description_ar: podcastData.description_ar || podcastData.description,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      updatedPodcast.thumbnail = finalThumbnailUrl;
      
      const { error } = await supabase.from('podcasts').update(updatedPodcast).eq('id', id);
  ...
  if (isLoading) {
    return <div className="p-8 text-center text-muted-foreground">Loading podcast data...</div>;
  }

  return (
    <div className="p-8 max-w-4xl mx-auto">
      <h1 className="text-3xl font-bold tracking-tight mb-8">Edit Podcast Show</h1>
