...
export default function TodaysEventContentPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  async function fetchData() {
    // Fetch Today's Event
    const { data: pData } = await supabase.from('todays_events').select('*').eq('id', id).single();
    if (pData) setTodaysEvent(pData);

    // Fetch associated videos
    const { data: vData } = await supabase.from('videos').select('*').eq('todays_event_id', id).order('created_at', { ascending: false });
  ...
  if (!todaysEvent) return <div className="p-8 text-red-500">Today's Event not found</div>;
  ...
  return (
    <div className="p-8">
      ...
      <div className="flex items-center gap-6 mb-12 bg-card p-6 rounded-xl border border-border">
        <img src={todaysEvent.thumbnail} alt={todaysEvent.title} className="w-32 h-32 object-cover rounded-lg shadow-md" />
        <div>
          <h1 className="text-3xl font-bold tracking-tight mb-2">{todaysEvent.title}</h1>
          <p className="text-muted-foreground max-w-2xl">{todaysEvent.description}</p>
        </div>
      </div>
      ...
      <div className="flex items-center justify-between mb-8">
        ...
        <Link 
          href={`/admin/todays-events/${id}/content/new`}
              ...
              <tr key={video.id} className="hover:bg-muted/50 transition-colors">
                ...
                <td className="p-4 text-right">
                  <div className="flex items-center justify-end gap-2">
                    <Link 
                      href={`/admin/todays-events/${id}/content/${video.id}/edit`}
            ...
            {videos.length === 0 && (
              <tr>
                <td colSpan={3} className="p-8 text-center text-muted-foreground">
                  No videos found for this todaysEvent. Click "Add Video" to create one.
                </td>
              </tr>
            )}
          </tbody>
