...
export default function AddTodaysEventVideoPage({ params }: { params: Promise<{ id: string }> }) {
  const router = useRouter();
  const supabase = createClient();
  const { id: todays_event_id } = use(params);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const newVideo = {
      ...
      todays_event_id: todays_event_id,
    };
        ...
        const upload = new tus.Upload(selectedFile, {
          ...
          onSuccess: async function () {
            ...
            if (dbError) {
               ...
               window.location.href = `/admin/todays-events/${todays_event_id}/content`;
            }
          },
        });
    ...
    <div className="p-8 max-w-4xl mx-auto">
      <div className="mb-6">
        <button onClick={() => router.back()} className="flex items-center gap-2 text-muted-foreground hover:text-white transition-colors text-sm">
          ...
          Back to Today's Event Content
        </button>
      </div>

      <h1 className="text-3xl font-bold tracking-tight mb-8">Add Episode to Today's Event</h1>
