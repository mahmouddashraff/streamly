...
export default function AdminTodaysEventsPage() {
  ...
  async function fetchTodaysEvents() {
    const { data } = await supabase.from('todays_events').select('*').order('created_at', { ascending: false });
  ...
  const handleDelete = async (id: string, title: string, thumbnail: string) => {
    if (confirm(`Are you sure you want to delete "${title}"?`)) {
      ...
      if (!error) {
        ...
        alert("Error deleting todaysEvent: " + error.message);
      }
    }
  };
      ...
      <div className="bg-card border border-border rounded-xl overflow-hidden">
        <table className="w-full text-left text-sm">
          <thead className="bg-muted text-muted-foreground border-b border-border">
            <tr>
              <th className="p-4 font-medium">Today's Event</th>
          ...
          <tbody className="divide-y divide-border">
            {todaysEvents.map((todaysEvent) => (
              <tr key={todaysEvent.id} className="hover:bg-muted/50 transition-colors">
                <td className="p-4">
                  <div className="flex items-center gap-3">
                    <img src={todaysEvent.thumbnail} alt={todaysEvent.title} className="w-16 h-10 object-cover rounded hidden sm:block" />
                    <div className="font-medium text-white">{todaysEvent.title}</div>
                  </div>
                </td>
                <td className="p-4 text-muted-foreground hidden md:table-cell">
                  <div className="max-w-[300px] truncate">{todaysEvent.description}</div>
                </td>
                <td className="p-4">
                  <span className={`px-2 py-1 rounded text-xs font-medium ${todaysEvent.published ? 'bg-green-500/10 text-green-500' : 'bg-yellow-500/10 text-yellow-500'}`}>
                    {todaysEvent.published ? 'Published' : 'Draft'}
                  </span>
                </td>
                <td className="p-4 text-right">
                  <div className="flex items-center justify-end gap-2">
                    <Link
                      href={`/admin/todays-events/${todaysEvent.id}/content`}
                    ...
                    <Link 
                      href={`/admin/todays-events/${todaysEvent.id}/edit`}
                    ...
                    <button 
                      onClick={() => handleDelete(todaysEvent.id, todaysEvent.title, todaysEvent.thumbnail)}
            ...
            {todaysEvents.length === 0 && (
              <tr>
                <td colSpan={4} className="p-8 text-center text-muted-foreground">
                  No todaysEvents found. Click "Add Today's Event Show" to create one.
                </td>
              </tr>
            )}
          </tbody>
