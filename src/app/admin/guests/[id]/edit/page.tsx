...
export default function EditGuestPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    async function fetchData() {
      const { data, error: fetchError } = await supabase.from('guests').select('*').eq('id', id).single();
      if (data && !fetchError) {
        setFormData({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { error: dbError } = await supabase.from('guests').update({
        ...formData,
  ...
  if (isLoading) {
    return <div className="p-8 text-center text-muted-foreground">Loading guest data...</div>;
  }

  return (
    <div className="p-8 max-w-2xl mx-auto">
      <h1 className="text-3xl font-bold tracking-tight mb-8">Edit Guest</h1>
