...
export default async function SoonPage() {
  ...
  let soon: any[] = [];
  try {
    const { data } = await supabase.from('soon').select('*').eq('published', true).order('created_at', { ascending: false });
    if (data) soon = data;
  } catch (e) {
    console.error("Soon table not available");
  }
    ...
    <div className="container mx-auto px-4 md:px-8 py-24 min-h-[80vh]">
      ...
      {soon.length > 0 ? (
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-6">
          {soon.map((p: any) => (
            <Link href={`/soon/${p.id}`} key={p.id} className="group flex flex-col items-center text-center">
