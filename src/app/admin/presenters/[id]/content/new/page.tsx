...
export default function AddPresenterVideoPage({ params }: { params: Promise<{ id: string }> }) {
  const router = useRouter();
  const supabase = createClient();
  const { id: presenter_id } = use(params);
        ...
        const upload = new tus.Upload(selectedFile, {
          ...
          onSuccess: async function () {
            ...
            if (dbError) {
               ...
               window.location.href = `/admin/presenters/${presenter_id}/content`;
            }
          },
        });
    ...
    <div className="p-8 max-w-4xl mx-auto">
      <div className="mb-6">
        <button onClick={() => router.back()} className="flex items-center gap-2 text-muted-foreground hover:text-white transition-colors text-sm">
          ...
          Back to Presenter Content
        </button>
      </div>

      <h1 className="text-3xl font-bold tracking-tight mb-8">Add Video to Presenter</h1>
